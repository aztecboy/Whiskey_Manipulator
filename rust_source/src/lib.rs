...
*/
pub fn attach(pid:usize,byte_order:u8)->Result<WindowsProcess,&'static str>{
    use error::attach::*;
    use error::*;
    use whiskclib::*;
    ...
    match unsafe{attach_process(pid)}{
...
*/
pub fn dettach(windows_process:&WindowsProcess)->Result<(),&'static str>{
    use error::dettach::*;
    use error::*;
    use whiskclib::*;
...
pub fn read_32bit_address(windows_process:&WindowsProcess,address:usize)->Result<u32,&'static str>{
    ...
    //println!("{}",address_struct.data32);
    //println!("{}",address_struct.data);
    //println!("{}",address_struct.error_code);
    match address_struct.error_code{
        INVALID_ADDRESS_ERRORCODE=>return Err(INVALID_ADDRESS),
